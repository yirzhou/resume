% !TEX program = xelatex

\documentclass{resume}
%\usepackage{zh_CN-Adobefonts_external} % Simplified Chinese Support using external fonts (./fonts/zh_CN-Adobe/)
%\usepackage{zh_CN-Adobefonts_internal} % Simplified Chinese Support using system fonts

\begin{document}
\pagenumbering{gobble} % suppress displaying page number

\name{Yiren Zhou}

\basicInfo{
  \email{yirenzho@andrew.cmu.edu} \textperiodcentered\
  \github[yirzhou]{https://yirzhou.github.io/} \textperiodcentered\
  \linkedin[yirzhou]{https://www.linkedin.com/in/yirzhou/} \textperiodcentered\
  \phone{650-625-7637} \textperiodcentered\
  \location{Mountain View, CA}
}

\section{\faCogs\ Technical Skills}
\begin{itemize}[parsep=0.5ex]
  \item Languages: Python, Java, Go, JavaScript, C++, Rust, SQL
  \item Technologies/Tools: Git, Linux, Docker, AWS, NodeJS, CMake
\end{itemize}

\section{\faGraduationCap\ Education}
\datedsubsection{\textbf{Carnegie Mellon University}, Mountain View, CA}{08/2021 -- 12/2022}
\textit{Master of Science in Software Engineering}
\begin{itemize}
  \item {\textbf{Relevant courses:} Foundation of Computer Systems, Machine Learning for Signal Processing, Foundations in Software Engineering}
\end{itemize}

\datedsubsection{\textbf{University of Waterloo}, Waterloo, ON, Canada}{09/2016 -- 04/2021}
\textit{Bachelor of Applied Science in Computer Engineering,} GPA 4.0/4.0 (91.86\%)
\begin{itemize}
  \item {\textbf{Award:} President's Research Award, for work on the CheckerFramework, a static analysis tool for Java code.}
  \item {\textbf{Relevant courses:} Programming for Performance, Algorithm Design and Analysis, Distributed Computing, Database Systems, Compilers, Operating Systems, Cooperative and Adaptive Algorithms, Probability Theory and Statistics}
\end{itemize}

\section{\faUsers\ Work Experience}
\datedsubsection{\emph{Software Engineer Intern} \textperiodcentered\ \textbf{Bloomberg LP.} NY, United States}{09/2020 -- 12/2020}
Built DataLoader, a C++ data-fetching library to work with user-customizable protocols to serve millions of requests:
\begin{itemize}
  \item {Reduced network round-trips of data fetching via batching and caching of requests.}
  \item {Increased network throughput by utilizing coroutines and event-loops.}
  \item {Delivered comprehensive tech talk on DataLoader to 50+ engineers.}
  \item {Completed full software development life cycle and deployed DataLoader in three months.}
\end{itemize}

\datedsubsection{\emph{Backend Developer Intern} \textperiodcentered\ \textbf{TextNow Inc.} ON, Canada}{09/2019 -- 12/2019}
Re-architected QoS Packet Tester, A microservice written in Go to measure call quality of 700K+ Android users:
\begin{itemize}
  \item {Increased scalability by utilizing goroutines to allow parallel processing of thousands of user requests.}
  \item {Improved services observability with distributed tracing using OpenTelemetry and Datadog API.}
  \item {Completed full software development life cycle and deployed QoS Packet Tester in three months.}
\end{itemize}

\datedsubsection{\emph{Software Engineer Intern} \textperiodcentered\ \textbf{Capital One Financial Corporation} ON, Canada}{01/2019 -- 04/2019}
Developed PCI Crypto, a Python application to encrypt banking data amounting to millions of dollars in revenue:
\begin{itemize}
  \item {Achieved constant memory usage of encrypting large files by processing files in chunks.}
  \item {Built event-driven monitoring system to allow data owners to receive timely feedback.}
        % \item {Automated PCI files validation before file processing and encryption to reduce runtime errors.}
  \item {Coordinated collaboration among five teams and deployed PCI Crypto to production within two months.}
\end{itemize}

\datedsubsection{\emph{Web Developer Intern} \textperiodcentered\ \textbf{Nokia Corporation} ON, Canada}{05/2018 -- 08/2018}
Developed Nuage SD-WAN Portal, an application which automates deployment and management of large-scale networks:
\begin{itemize}
  \item {Reduced labor intensity of feature testing and development of SD-WAN portal by 70\%
        by building a proprietary tool to automate configuration of large-scale networks.}
  \item {Built, optimized, and shipped an array of UI features (e.g., navigation bars, forms) of SD-WAN Portal in Angular.}
  \item {Completed development within three months and released automation tool to 10+ developers.}
\end{itemize}

\section{\faCodeFork\ Projects}
\begin{itemize}[parsep=0.5ex]
  \item \link[DCache]{https://drive.google.com/file/d/1Svd-wbXZiO84QmVKnKBp1cClJb2uvtWT/view?usp=sharing}: A distributed, P2P, and geolocation-aware mobile storage network.
  \item \link[rsbday]{https://github.com/yirzhou/IsolatedSoul/tree/main/rsbday}: A command-line tool for storing birthdays and sending reminders, written in Rust.
  \item \git[Grafana Datasource Plugin for Datadog]{https://github.com/Enflick/grafana-datasource-datadog/pull/7}: An analytics and visualization software for big data, for use in Grafana.
  \item \repo[gpgodzilla]{https://pypi.org/project/gpgodzilla/}: A Python module to allow users to manipulate arbitrary-sized files with constant memory space.
  \item \link[YNOT Blogs]{http://ynotblogs.herokuapp.com/blogs}: A blogging application with user authentication, authorization, and search in NodeJS and MongoDB.
        % \item \link[zhouyiren.rocks]{https://zhouyiren.rocks}: A responsive and minimalist portfolio using ReactJS, React Router, and Github GraphQL API.
\end{itemize}

% Reference Test
%\datedsubsection{\textbf{Paper Title\cite{zaharia2012resilient}}}{May. 2015}
%An xxx optimized for xxx\cite{verma2015large}
%\begin{itemize}
% \item main contribution
%\end{itemize}

%% Reference
%\newpage
%\bibliographystyle{IEEETran}
%\bibliography{mycite}
\end{document}
